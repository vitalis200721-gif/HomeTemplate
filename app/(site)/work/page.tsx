    ...
    <section className="py-10 md:py-14">
      <div className="mx-auto max-w-6xl px-4">
        <div className="flex items-end justify-between gap-6">
          <div>
            <div className="text-xs font-semibold tracking-wide text-white/60">Work</div>
            <h1 className="mt-2 text-3xl font-semibold md:text-4xl">Selected projects</h1>
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-white/65">
              Case studies are mock content driven from <code className="rounded bg-white/5 px-1 py-0.5">lib/data.ts</code>.
              Swap the copy and you’re production-ready.
            </p>
          </div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {data.projects.map((p) => (
            <Link
              key={p.slug}
              href={`/work/${p.slug}`}
              className="group relative overflow-hidden rounded-3xl border border-white/10 bg-neutral-900/40 p-6 transition hover:bg-white/5"
            >
